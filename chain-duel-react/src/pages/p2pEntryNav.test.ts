...
describe('moveNavFocus none state', () => {
  ...
  it('starts unselected and enters on horizontal press', () => {
    expect(
      moveNavFocus({ kind: 'none' }, 'left', tournament, sessionNavIdx)
    ).toEqual({
      kind: 'payment',
      idx: 0,
    });
    expect(
      moveNavFocus({ kind: 'none' }, 'right', tournament, sessionNavIdx)
    ).toEqual({
      kind: 'payment',
      idx: 1,
    });
  ...
  it('enters on down from unselected (gamepad joystick)', () => {
    expect(
      moveNavFocus({ kind: 'none' }, 'down', tournament, sessionNavIdx)
    ).toEqual({
      kind: 'payment',
      idx: 0,
    });
  ...
  it('ignores up while unselected', () => {
    expect(
      moveNavFocus({ kind: 'none' }, 'up', tournament, sessionNavIdx)
    ).toEqual({
      kind: 'none',
    });
  });
  ...
  it('stays on payment when moving up from the top row', () => {
    expect(
      moveNavFocus({ kind: 'payment', idx: 0 }, 'up', tournament, sessionNavIdx)
    ).toEqual({
      kind: 'payment',
      idx: 0,
    });
...
describe('moveNavFocus duel mode', () => {
  ...
  it('moves down from duel session through format to open game menu', () => {
    ...
    expect(
      moveNavFocus({ kind: 'duelFormat' }, 'down', tournament, sessionNavIdx)
    ).toEqual({
      kind: 'start',
    });
  });
  ...
  it('moves up from footer start to duel format', () => {
    expect(
      moveNavFocus({ kind: 'start' }, 'up', tournament, sessionNavIdx)
    ).toEqual({
      kind: 'duelFormat',
    });
    expect(
      moveNavFocus({ kind: 'duelFormat' }, 'up', tournament, sessionNavIdx)
    ).toEqual({
      kind: 'session',
      idx: 0,
    });
...
describe('moveNavFocus tournament bracket grid', () => {
  ...
  it('moves vertically within the players grid', () => {
    ...
    expect(
      moveNavFocus({ kind: 'players', idx: 2 }, 'up', tournament, sessionNavIdx)
    ).toEqual({
      kind: 'players',
      idx: 0,
    });
